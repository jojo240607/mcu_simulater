...
fn hover_60s_env() {
    ...
    for step in 0..15_000u64 {
        ...
        // 注入逼真传感器读数（噪声化，与 SIL 控制律同源）
        {
            ...
            st.rc_ch[4] = 2000.0;
            // ---- 验收模式：LOITER（位置环开启）--------------------------------
            // 模式通道 ch[5]（SBUS 第 6 通道三位开关，固件 `sbus.rs` 解析）：
            //   <600 → 0 = STABILIZE | <1400 → 1 = ALT_HOLD | >=1400 → 2 = LOITER
            //
            // 本测例的验收目标是"**抗 3 级风**"（风场 `WindConfig::beaufort3()`），
            // 而"抗风"的含义是**守住位置** ⇒ 必须 LOITER。
            // 若停在 STABILIZE/ALT_HOLD（位置环被旁路）则水平位置自由积分，
            // H 场同风场实测该模式下 60s 漂 51.71m，"漂移<8m"判据在该模式下无意义。
            //
            // **不需要 USB**：模式经遥控器通道下发（真机上飞行员必须能自己切模式，
            // 这也是地面站断链时唯一的手段）。固件侧已接唯一真源
            // `flyctrl_core::flightmode::mode_from_rc_switch`（RC 优先、失联回退地面站）。
            st.rc_ch[5] = 2000.0; // → mode 2 = LOITER

            // ---- 【两场输入等价】磁力计来源 ----
            //
            // 原行为：虚拟外设的磁力计由 `st.att`（世界→机体四元数）旋转世界地磁推导
            // （`data_source.rs:538-547`），而 `st.att` 只由 `scn.write_state()` 按
            // **场景解析真值**写入（`scenario.rs:421`）。本测例却用 SimLoop 物理引擎
            // 推进机体，且**从不把实际姿态写回 `st.att`**（grep 实测：
            // mcu_simulater/tests/*.rs 中无一处写 st.att / st.mag）
            // ⇒ 固件永远收到「机体水平」的**假磁场**，与物理引擎里真实的姿态对打。
            // 而 H 场（FlyController）是用**真实姿态**推导磁力计的。
            //
            // 修法：不自己重算磁场（易引入四元数约定错误），而是**直接用物理引擎的
            // 磁采样**经 `st.mag` 覆盖通道喂入 ⇒ 两场磁输入**同源**（同一个世界场、
            // 同一套 SensorConfig 噪声/硬铁）。这样 H 场验证过的磁相关结论才可被
            // M 场验收。
            //
            // 实测影响（2.5m/s 侧风+阵风+湍流 60s）：
            //   假磁场（原行为）    max|roll| 20.68°  max|pitch| 35.30°   FAIL
            //   关磁锚（对照）      max|roll|  6.23°  max|pitch| 16.49°   PASS(姿态)
            //   本修法（同源磁输入）max|roll|  6.23°  max|pitch| 16.49°   PASS(姿态)
            // ⇒ 与"关磁锚"逐位相同，因为**物理正确的磁输入与真实姿态自洽**
            //   ⇒ EKF 算出的 yaw 误差 ≈ 0 ⇒ 不做修正，等同"没有假磁场在捣乱"。
            //   这正是期望结果：假磁场那条病治好了。
            //   残余（16.49° vs H 场同场景 4.63°）在**另一条输入通路**上，尚未定位。
            // **默认启用**（不是 A/B 开关）：两场磁输入同源是本测例有效性的前提。
            // 曾用 ZZ_MAG_FROM_PLANT 做过对照，结论已固化在下方。
    ...
    // - 水平漂移有界：本测例已切到 **LOITER（位置环开启）** ⇒ 位置是被保持的，
    //   该判据成立。<8m 不是拍的数：H 场**同风场同 60s**（位置环口径）实测 6.64m
    //   （`wind_turb_scan`，B3 上限 5.4m/s），留 ~17% 余量。
    //   ⚠️ 该判据只在位置环模式下有效：ALT_HOLD/STABILIZE 下位置自由积分
