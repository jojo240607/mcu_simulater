...
/// 直接读取 App SDK 日志 ring（LOG_RING=0x2000FED0，2048B）。条目格式：
/// [len][level][payload]，payload = "R/{level} {tick} {tag}: {msg}\n"。
/// 地址来自 `arm-none-eabi-nm app.elf | grep -E 'LOG_(RING|HEAD|TAIL)'`：
///   LOG_RING=0x2000FED0 LOG_HEAD=0x200116E8 LOG_TAIL=0x200116EC
/// （App 重建后需按 nm 同步更新。）
fn scan_log_ring(m: &mut Machine) -> String {
    const RING_ADDR: u64 = 0x2000FED0;
    const HEAD_ADDR: u64 = 0x200116E8;
    const TAIL_ADDR: u64 = 0x200116EC;
