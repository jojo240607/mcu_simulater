//! M4 验收测试：DMA2 内存到内存传输端到端（MEM2MEM → TC 中断 → handler 校验数据）。
//!
//! 复用 firmware/dma2_demo 固件（与 dma_demo 同构，仅 DMA2 @ 0x40026400 + IRQ56）：
//! 1. 测试预置源数组 SRC@0x20000100（16 字）；
//! 2. 固件配置 DMA2 Stream0：PAR=SRC、M0AR=DST@0x20000200、NDTR=16、字宽、
//!    PINC+MINC、DIR=内存到内存、TCIE，写 EN 启动；
//! 3. 仿真器首个块 tick 判定完成：置 TCIF、挂起 IRQ56、run 间隙搬运 SRC→DST；
//! 4. DMA2_Stream0_IRQHandler：校验 DST==SRC → G_DMA_OK；清 LIFCR → G_DMA_TC++；
//! 5. 主线写 G_DONE。
//!
//! 期望结果区：
...
#[test]
fn m4_dma2_mem2mem_transfer_end_to_end() {
    let mut m = load_machine();

    // 预置源数组（16 个字，测试可辨识值，与 dma1 用例区分）
    ...
    assert_eq!(read_u32(&mut m, G_DMA_TC), 1, "DMA2 完成中断应执行 1 次");
    assert_eq!(read_u32(&mut m, G_DMA_OK), 1, "handler 内 DST==SRC 校验应通过");

    // 2) 直接内存校验：DMA2 搬运结果 DST == SRC
    for (i, v) in src.iter().enumerate() {
